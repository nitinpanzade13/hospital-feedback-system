// ===========================================
// FORM_SCRIPTS_PEDIATRICS_SYNC_INIT_MR.gs
// Deploy this to the Pediatrics - Marathi Form
...
const DEPARTMENT_NAME = "Pediatrics";
const LANGUAGE_CODE = "marathi"; // english | hindi | marathi
...
function captureFormResponse(e) {
  try {
    ...
    const patientName = pickResponse(allResponses, NAME_QUESTION_KEYS.marathi) || "Anonymous";
...
function ensureNameQuestionLanguage(form) {
  const targetTitle = NAME_QUESTION_KEYS.marathi[0];
  const englishKeys = NAME_QUESTION_KEYS.english || [];
  const marathiKeys = NAME_QUESTION_KEYS.marathi || [];
  ...
  for (let item of items) {
    ...
    if (marathiKeys.indexOf(title) >= 0) {
      return;
    }

    if (englishKeys.indexOf(title) >= 0) {
      item.setTitle(targetTitle);
      Logger.log(`✏️ Renamed patient name field to Marathi: ${targetTitle}`);
      return;
    }
  }
}
